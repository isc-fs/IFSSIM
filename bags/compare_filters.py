...
def main() -> None:
    ...
    # Anchor each stream to its first sample so all three start at
    # (0,0,0).  GT and filters all use different world origins;
    # post-anchor, the residual is what we want.
    def anchor(s):
        if not s:
            return s
        x0, y0, yaw0 = s[0][1], s[0][2], s[0][3]
