...
`project_glim_next.md` memory entry can be removed (it'll be stale). Update `project_immediate_gaps.md` and `project_ifssim.md` to reflect FAST-LIO as the live localization layer.

## 15. Post-mortem (2026-04-27): GLIM did not work — pivoted to FAST-LIO2

After completing steps 1-5 (Dockerfile build chain, glim_rosnode spawn, monotonic IMU stamps, bridge static TFs, pipeline frame rename, Odometria_perfecta deletion), step 6 validation drove the simulator and compared GLIM's published pose against `/fsds/testing_only/odom` (ground truth). Two failures, in order:

**Failure 1 — divergence at speed.** Autonomous drive at peak ~10 m/s:

| t (s) | GLIM dist (m) | GT dist (m) | drift (m) |
|---|---|---|---|
| 0 | 0.00 | 0.00 | 0.001 |
| 3 | 1.05 | 3.19 | 2.15 |
| 6 | 1.16 | 12.14 | 10.98 |
| 9 | 1.32 | 26.73 | 25.41 |
| 15 | 7.84 | 64.65 | 56.81 |
| 21 | 24.57 | 105.30 | 80.73 |

GLIM tracked the first 3 s, then drift exploded. After ~21 s its pose froze (24.57 m) while the car kept moving — it had effectively lost the car. Final relative drift: 76 % of GT distance.

**Failure 2 — node death under load.** During a follow-up validation drive, `glim_rosnode` exited mid-run (foxglove_bridge logs show all `/glim_ros/*` topic channels removed at the same instant). Likely cause: the container's 1500 MB memory limit insufficient for GTSAM + sub-mapping + global-mapping submap storage during sustained operation. Restarting the node restored it briefly, then it died again.

**Root cause analysis (best read):**

- IMU validation warnings persisted from step 2 onward: `IMU prediction is not good`, `IMU better ratios rot=0.08-0.23`. GLIM's own self-report that the IMU integration was unreliable for this data.
- The CPU `OdometryEstimationCPU` module (mandated by the no-CUDA / real-car-compute-envelope decision) couldn't keep up with 200k pts/s LiDAR at 10 Hz on the container's CPU budget. Once the scan-matcher fell behind, the IMU prediction took over and accumulated unbounded error.
- Memory pressure from continual sub-mapping snapshot accumulation eventually crashed the node.

**Pivot decision:** FAST-LIO2 (Ericsii ROS2 port). Lighter deps (PCL + Eigen, no GTSAM), simpler IKFoM filter, runs comfortably on Raspberry Pi-class CPUs. The architectural work in steps 2.5, 3, 4, 5 (frame rename, bridge static transforms, monotonic stamps, dual-pose collapse) is library-agnostic and stays. Only steps 1 (Dockerfile build chain) and 2 (node launch + config) get swapped.

The branch `feat/28-glim-localization` continues with FAST-LIO2 commits. Will be renamed `feat/N-fast-lio-localization` (or similar) at PR-merge time so history reflects the pivot.
