...
%Función propiedades del agua. A partir de una tabla de datos se iteran las
%propiedades para una temperatura de entrada dada. Interpolación lineal.

%Se carga la matrix de propiedades el agua.
load('RE_CS_220901_APropiedadesAgua.mat','B');
...
%Planteamiento secundario. Se intuye que deberia ser lo mismo.
ro_wat=interp1(B(:,1),B(:,2),Teval_wat);
cp_wat=interp1(B(:,1),B(:,3),Teval_wat);cp_wat=cp_wat*1e3;
mu_wat=interp1(B(:,1),B(:,4),Teval_wat);mu_wat=mu_wat*1e-6;
k_wat=interp1(B(:,1),B(:,6),Teval_wat);k_wat=k_wat*1e-3;
Pr_wat=interp1(B(:,1),B(:,9),Teval_wat);
end
