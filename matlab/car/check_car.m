...
% HEAT, NOT A RATING. The cell has no published maximum current, so there is
% nothing to compare against; what there is, is a temperature rise that
% depends entirely on how long the current is held. Reported per event.
fprintf('  self-heating at %.1f A per cell: %.1f W, %.2f K/s\n', ...
        K.CellAmpsAtOperating, K.CellWatts, K.CellKperSec);
fprintf('     %5.1f K over a 4 s acceleration run\n',  K.CellKperSec*4);
fprintf('     %5.1f K over a 22 s autocross lap\n',    K.CellKperSec*22);
fprintf('     %5.1f K over 5 minutes of endurance\n',  K.CellKperSec*300);
[ok,nwarn] = band(ok,nwarn,'cell rise over an accel run', K.CellKperSec*4,'K', 0, 15, ...
    'A burst this short is thermally cheap; if this is large the current is enormous or the resistance is.');
[ok,nwarn] = band(ok,nwarn,'cell rise over a lap', K.CellKperSec*22,'K', 0, 40, ...
    'Held for a whole lap the same current is a different question. The cells start a lap warm, and 80 C is where the datasheet stops testing.');
