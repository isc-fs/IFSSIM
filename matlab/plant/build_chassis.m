...
%
%   The block that makes the model move. Everything else feeds it forces; this
%   integrates them into a trajectory.
%
...
%                        state --------------------------------------- +
%
%   The state lives in Unit Delay blocks, visible on the canvas, so a reader can
%   see what the model remembers. The equations live in ONE MATLAB Function
%   block rather than sixty primitives, because Newton-Euler expressed as
%   Sum/Gain/Product blocks is not more readable than Newton-Euler expressed as
%   Newton-Euler — it is the same six lines with worse typography and forty
%   opportunities for a mis-wired signal. Subsystems where an engineer really
%   does want to reshape the maths block-by-block (tyre, suspension, motor) are
%   a different judgement.
%
%   DISCRETE, forward Euler, at the model's fixed step. No continuous states,
%   no algebraic loop: the update reads LAST step's state, so the loop is broken
%   by construction rather than by a solver tolerance.
...
% inputs 6..9 are LAST step's state, read back from the delays. This is what
...
add_block('built-in/Note',[name '/Notes'],'Position',[40 700], ...
    'Text', chassis_notes(P), 'HorizontalAlignment','left');
...
%% =======================================================================
function t = chassis_notes(P)
t = sprintf([ ...
 'IFSSIM_CHASSIS — 6-DOF rigid body\\n\\n' ...
 'OWNER: dynamics\\n\\n' ...
 'Newton-Euler in the body frame, forward Euler at 1/960 s. State is in the Unit\\n' ...
 'Delay blocks so it is visible on the canvas; the equations are in one MATLAB\\n' ...
 'Function block because sixty primitives express the same six lines with worse\\n' ...
 'typography and forty chances to mis-wire a signal.\\n\\n' ...
 'PARAMETERS come from IFSSIM_P (settings.json). Mass %.0f kg.\\n\\n' ...
 'INERTIA IS AN ASSUMPTION: Ixx %.0f, Iyy %.0f, Izz %.0f kg m^2. settings.json\\n' ...
 'has no inertia and UE only scales whatever its physics asset computes, so there\\n' ...
 'is nothing authoritative to read. Izz sets yaw response, which is what the\\n' ...
 'controller is tuned against — a 20%%%% error there will read as a gain problem.\\n' ...
 'Measure it (bifilar pendulum, or CAD mass properties) and move it into\\n' ...
 'settings.json.\\n\\n' ...
 'TWO THINGS THAT ARE EASY TO GET WRONG AND ARE DONE DELIBERATELY HERE:\\n\\n' ...
 '  1. accel_proper EXCLUDES gravity. It is what an accelerometer reads. The\\n' ...
 '     simulator currently finite-differences a world velocity and adds +g,\\n' ...
 '     which is where the EKF''s missing Coriolis terms came from.\\n\\n' ...
 '  2. The Coriolis term -omega x v is present. Body-frame rates are not\\n' ...
 '     inertial. Dropping it is exactly the bug that made lateral velocity\\n' ...
 '     drift during sustained cornering.\\n\\n' ...
 'The quaternion is renormalised every step. Forward Euler walks off the unit\\n' ...
 'sphere and does it silently.'], ...
 P.Mass, P.Assumed.Ixx, P.Assumed.Iyy, P.Assumed.Izz);
end
