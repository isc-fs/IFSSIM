...
%
%   Motor envelope, drivetrain, and a battery with state of charge.
%
%   THE TORQUE SPLIT IS WRITTEN AS A VECTOR, not baked into the maths. This car
%   is RWD today, but the stated reason this platform exists is to let someone
%   try four in-wheel motors with torque vectoring. That experiment should be an
%   edit to one line here, not a rewrite — and it must not be silently an RWD
%   experiment, which is what would happen if the split were implicit.
...
t = sprintf([ ...
 'IFSSIM_POWERTRAIN\\n\\nOWNER: powertrain\\n\\n' ...
 'Motor %.0f Nm / %.0f kW, single reduction %.3f, efficiency %.2f.\\n' ...
 'Pack %.0f V full, %.0f V empty, %.1f kWh.\\n\\n' ...
 'REGEN IS POWER LIMITED, NOT TORQUE LIMITED. MaxRegenPower is the cell input\\n' ...
 'current cap. At 10 m/s it allows about %.0f Nm at the motor against a %.0f Nm\\n' ...
 'envelope — a factor of %.1f. Regen is the ONLY service braking this car has,\\n' ...
 'so this limit, not the motor torque figure, is what sets how hard it can stop.\\n\\n' ...
 'DRIVE AND REGEN ARE SUMMED. Chaos discards one of them — its wheel solver\\n' ...
 'picks braking or driving by magnitude, so a simultaneous command loses a\\n' ...
 'channel silently. Summing is what the real inverter does.\\n\\n' ...
 'THE TORQUE SPLIT IS A VECTOR, [0 0 0.5 0.5] for RWD. Change it to try four\\n' ...
 'in-wheel motors or torque vectoring. It is written out explicitly so that\\n' ...
 'experiment cannot silently remain an RWD experiment — which is exactly what\\n' ...
 'happened in the UE plant, where the front wheels discarded applied torque\\n' ...
 'because their combine method was left at the Chaos default.\\n\\n' ...
 'BATTERY PARAMETERS ARE HARVESTED FROM matlab/IFS_Sim, NOT settings.json. That\\n' ...
 'model also carried mass 237 kg and tyre radius 0.30 m, both of which disagree\\n' ...
 'with settings.json — so it is a different car or a different year. The pack\\n' ...
 'numbers are probably right, but confirm them and move them into settings.json.\\n\\n' ...
 'Open-circuit voltage is linear in SoC and pack resistance is a guess. Nothing\\n' ...
 'downstream consumes terminal voltage yet, but a battery whose voltage never\\n' ...
 'sags will flatter any power study.'], ...
 P.MotorMaxTorque, P.MotorMaxPower/1000, P.GearRatio, P.DrivetrainEfficiency, ...
