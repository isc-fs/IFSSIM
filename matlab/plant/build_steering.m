...
t = sprintf([ ...
 'IFSSIM_STEERING                     OWNER: dynamics / controls\\n\\n' ...
 'Max road-wheel angle %.1f deg. Ackermann fraction %.0f%%%% — ASSUMPTION,\\n' ...
 'the IFS-08 steering-arm geometry is not recorded in this repo.\\n\\n' ...
 'The command is the SINGLE-TRACK angle; Ackermann splits it across the front\\n' ...
 'wheels. The autonomy plans against a kinematic bicycle, so this keeps both\\n' ...
 'geometries in agreement.\\n\\n' ...
 'Actuator modelled but defaulted off (rate %.0f rad/s, lag %.0e s) — better no\\n' ...
 'lag than the wrong lag.\\n\\n' ...
 'TO RECONCILE: settings.json %.0f deg, pipeline 18.2 deg, and the real 5:1\\n' ...
 'ratio with a +/-60 deg column clamp implies ~12 deg. Three sources, no\\n' ...
 'agreement — a calibration question, but somebody should settle it.\\n\\n' ...
 'Edit build_steering.m, not this model — it is regenerated.'], ...
 rad2deg(P.Derived.MaxSteerAngleRad), P.Assumed.AckermannFraction*100, ...
