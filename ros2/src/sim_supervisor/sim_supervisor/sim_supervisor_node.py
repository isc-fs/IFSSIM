...
class SimSupervisorNode(LifecycleNode):
    ...
    def _on_ctrl_cmd(self, msg: Twist) -> None:
        ...
        is applied only in AS_DRIVING. throttle = linear.x, steering =
        angular.z (both [-1, 1]); the sim bridge consumes the normalised
        ControlCommand directly (no scaling — that's the uDV's job on the
        real car).
        """
